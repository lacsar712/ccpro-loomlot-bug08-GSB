<script>
  ...
  async function load() {
    ...
    try {
      ...
      // 旁路计数：与色牢度页按染程筛选的 total 对账，两者必须一致
      const counts = await Promise.all(
        rows.map((lot) => api(`/fastness-checks/by-lot-count?dyeLotId=${lot.id}`))
      );
      checkCounts = Object.fromEntries(counts.map((c) => [c.dyeLotId, c.count]));
